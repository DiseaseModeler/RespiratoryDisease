...
```{r}
#| echo: false
# Lab results from local Excel file – COVID & influenza positivity
...
the_df <- data.frame(
  date       = df_lab$WEEK_ENDING_DATE,
  covid_prop = covid_proportion,
  flu_prop   = influenza_proportion
)
...
p_lab <- ggplot(the_df, aes(x = date)) +
  geom_line(aes(y = covid_prop, colour = "COVID-19"), na.rm = TRUE) +
  geom_line(aes(y = flu_prop,   colour = "Influenza"), na.rm = TRUE) +
  geom_point(aes(y = covid_prop, colour = "COVID-19")) +   # ← CHANGED
  geom_point(aes(y = flu_prop,   colour = "Influenza")) +  # ← CHANGED
  scale_x_date(date_breaks = "1 month", date_labels = "%b %Y") +
  scale_y_continuous(limits = c(0, 1)) +     # ← ADD THIS LINE
  scale_colour_manual(
    values = c(
      "Influenza" = "#1B9E77",  # same green as other tabs
      "COVID-19"  = "#D95F02"   # same orange as other tabs
    ),
    breaks = c("Influenza", "COVID-19"),
    name   = "Pathogen"
  ) +
  labs(
    title   = "Lab test positivity (CCHMC SALT toy data)",
    x       = "Week ending",
...
```{r}
#| echo: false
targets_6_bh <- tibble::tibble(county = c("Butler","Hamilton"),
                               fips   = c("39017","39061"))
