...
make_ed_panels <- function(raw_vutn, target_state = "OHIO") {
  ...
  yrng_us <- range(df_us$pct, na.rm = TRUE)
  yrng_st <- range(df_st$pct, na.rm = TRUE)

last_lab_us <- format(max(df_us$week_end, na.rm = TRUE), "%b %d, %Y")
last_lab_st <- format(max(df_st$week_end, na.rm = TRUE), "%b %d, %Y")

  make_panel <- function(dat, title_txt, yrng) {
  ...
  list(
    p_us = make_panel(df_us, paste0("United States through week ending ", last_lab_us), yrng_us),
    p_state = make_panel(df_st, paste0(str_to_title(target_state), " through week ending ", last_lab_st), yrng_st)
  )
}
...
make_panel <- function(dat, raw_col, roll_col, virus_label, col_hex) {
  ...
  dat_plot <- dat %>%
    transmute(
      ...
      roll = .data[[roll_col]],
      text_raw = sprintf(
        "Week ending: %s<br>%s (weekly): %.3f",
        format(week_end, "%b %d, %Y"),
        virus_label,
        raw
      ),
      text_roll = sprintf(
        "Week ending: %s<br>%s (rolling median): %.3f",
        format(week_end, "%b %d, %Y"),
        virus_label,
        roll
      )
  ...
  plotly::plot_ly(dat_plot, x = ~week_end) %>%
    ...
    plotly::layout(
      title = list(
        text = paste0("<b>", virus_label, "</b>"),
        font = list(family = "Arial", size = 18)
      ),
      xaxis = list(
        title = "<b>Week ending</b>",
        font = list(family = "Arial", size = 15),
        #titlefont = list(family = "Arial Black"),
        tickformat  = "%b %Y",      # axis ticks (monthly)
        hoverformat = "%b %d, %Y",   # hover shows full date
        tickangle   = -45,          # <- rotate x-axis labels
        dtick       = "M2",     # <- every 2 months
        tick0 = "2023-10-01"
      ),
      yaxis = list(
        title = "<b>Admissions per 100k</b>",
        font = list(family = "Arial", size = 15),
        #titlefont = list(family = "Arial Black"),
        range = yrng
      ),
      ...
      showlegend = FALSE   # removes legend entirely
...
# Ohio: Flu admissions vs influenza wastewater

```{r}
#| echo: false

# -------------------------
# 1) Ohio flu admissions (weekly)
# -------------------------
df_flu_adm_oh <- DATA$mpgq %>%
  transmute(
    week_end     = as.Date(parse_date_any(weekendingdate)),
    jurisdiction = toupper(trimws(as.character(jurisdiction))),
    Flu_adm_per100k = suppressWarnings(as.numeric(totalconfflunewadmper100k))
  ) %>%
  filter(
    jurisdiction == "OH",
    !is.na(week_end),
    week_end >= cutoff_date,
    !is.na(Flu_adm_per100k)
  ) %>%
  arrange(week_end)

# -------------------------
# 2) Region 6 influenza wastewater: sum across counties -> weekly
# -------------------------
#targets_r6 <- regions[["6"]]
#
#ww_flu_r6 <- prep_wastewater(
#  DATA$ymmh_oh, targets_r6,
#  "influenza|\\bflu\\b|\\biav\\b|influenza[_\\s-]*a|normalized|normalised|rolling|activity|wva|percentile|conc|concen|load|signal|copies"
#)

# Convert daily samples -> "week ending Saturday" to match CDC-style week ends
#ww_flu_weekly_r6 <- ww_flu_r6$county_daily %>%
#  mutate(
#    week_end = lubridate::floor_date(sample_date, "week", week_start = 7) + lubridate::days(6)
#  ) %>%
#  group_by(week_end) %>%
#  summarise(
#    ww_flu_sum = sum(value_num, na.rm = TRUE),
#    .groups = "drop"
#  ) %>%
#  filter(!is.na(week_end), week_end >= cutoff_date) %>%
#  arrange(week_end)

# -------------------------
# 2) Ohio influenza wastewater: sum across ALL regions/counties -> weekly
# -------------------------
targets_all <- dplyr::bind_rows(regions)   # all 1–8 regions stacked

ww_flu_all <- prep_wastewater(
  DATA$ymmh_oh, targets_all,
  "influenza|\\bflu\\b|\\biav\\b|influenza[_\\s-]*a|normalized|normalised|rolling|activity|wva|percentile|conc|concen|load|signal|copies"
)

ww_flu_weekly_all <- ww_flu_all$county_daily %>%
  mutate(
    week_end = lubridate::floor_date(sample_date, "week", week_start = 7) + lubridate::days(6)
  ) %>%
  group_by(week_end) %>%
...
# 3) Join + plot
# -------------------------
#df_join <- inner_join(df_flu_adm_oh, ww_flu_weekly_r6, by = "week_end")
df_join <- inner_join(df_flu_adm_oh, ww_flu_weekly_all, by = "week_end")

# --- Spearman correlation ----
cor_test <- cor.test(df_join$ww_flu_sum,
                     df_join$Flu_adm_per100k,
                     method = "spearman")

rho   <- unname(cor_test$estimate)   # Spearman rho
p_val <- cor_test$p.value
#rho_lab <- sprintf("Spearman rho = %.2f (p = %.3f)", rho, p_val)
rho_lab <- sprintf("Spearman rho = %.2f (p = %.2e)", rho, p_val)


p_flu_adm_vs_ww <- ggplot(
  df_join,
  aes(
    x = ww_flu_sum,
    y = Flu_adm_per100k,
    text = sprintf(
      "Week ending: %s<br>WW Influenza sum across state: %.2f<br>OH Flu admissions per 100k: %.3f",
      format(week_end, "%b %d, %Y"),
      ww_flu_sum,
      Flu_adm_per100k
    )
  ...
  geom_smooth(
  data = df_join,
  aes(x = ww_flu_sum, y = Flu_adm_per100k, group = 1),
  method = "lm",
  se = FALSE,
  color = "red",
  alpha = 0.4,
  linewidth = 1.2,
  inherit.aes = FALSE) +   # <-- new fitted line
  #geom_smooth(method = "lm", se = FALSE) +
  labs(
    title = "Ohio influenza admissions vs Ohio influenza wastewater (sum across counties)",
    subtitle = paste0("Through ", format(max(df_join$week_end, na.rm = TRUE), "%b %d, %Y")),
    x = "Influenza wastewater (sum across Ohio counties; weekly)",
    y = "New influenza admissions per 100k (Ohio; weekly)"
  ) +
  theme_minimal(base_size = 13)



p_flu_adm_vs_ww_plotly <- gg_to_plotly(p_flu_adm_vs_ww) %>%
  layout(
    annotations = list(
      list(
        x = 0.02, y = 0.98,           # relative to plot area (paper coords)
        xref = "paper", yref = "paper",
        text = rho_lab,              # "Spearman rho = ..."
        showarrow = FALSE,
        xanchor = "left",
        yanchor = "top",
        font = list(size = 12)
...
# ===== Statewide pooled influenza wastewater over time (WEEKLY) =====
df_pool <- ww_flu_weekly_all %>%
  transmute(
    week_end   = week_end,
    ww_flu_sum = ww_flu_sum
  )

p_flu_ww_time <- ggplot(
  df_pool,
  aes(
    x = week_end,
    y = ww_flu_sum,
    text = sprintf(
      "Week ending: %s<br>Statewide pooled influenza wastewater (weekly sum): %.3f",
      format(week_end, "%b %d, %Y"),
      ww_flu_sum
    )
  )
  ...
  scale_y_continuous(
    limits = c(0, max(df_pool$ww_flu_sum, na.rm = TRUE) * 1.05)
  ) +
  labs(
    title = "Statewide pooled influenza wastewater (weekly)",
    subtitle = paste0(
      "Through ",
      format(max(df_pool$week_end, na.rm = TRUE), "%b %d, %Y")
    ),
    x = "Date",
    y = "Influenza wastewater (weekly sum across all Ohio counties)"
  ) +
  theme_minimal(base_size = 13) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

p_flu_ww_time_plotly <- gg_to_plotly(p_flu_ww_time)
p_flu_ww_time_plotly
...
# ===== Ohio influenza admissions over time =====
p_flu_adm_time <- ggplot(
  df_flu_adm_oh,
  aes(
    x = week_end,
    y = Flu_adm_per100k,
    text = sprintf(
      "Week ending: %s<br>Ohio flu admissions per 100k: %.3f",
      format(week_end, "%b %d, %Y"),
      Flu_adm_per100k
    )
  )
  ...
  scale_y_continuous(
    limits = c(0, max(df_flu_adm_oh$Flu_adm_per100k, na.rm = TRUE) * 1.05)
  ) +
  labs(
    title = "Ohio influenza admissions over time",
    subtitle = paste0(
      "Through ",
      format(max(df_flu_adm_oh$week_end, na.rm = TRUE), "%b %d, %Y")
    ),
    x = "Date",
    y = "New influenza admissions per 100k (Ohio; weekly)"
  ) +
...
p_flu_adm_time_plotly <- gg_to_plotly(p_flu_adm_time)
